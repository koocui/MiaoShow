...
void didCompressH264(void * outputCallBackRefCon,void * sourceFrameRefCon,OSStatus status,VTEncodeInfoFlags infoFlage,CMSampleBufferRef sampleBuffer){
    if (status != 0) return;
    //采集的未编码是否准备好
    if (!CMSampleBufferDataIsReady(sampleBuffer)){
        NSLog(@"didCompress264 data is not ready");
        return;
    }
